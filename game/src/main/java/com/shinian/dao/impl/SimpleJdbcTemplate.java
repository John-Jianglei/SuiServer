...
public class SimpleJdbcTemplate {
	...
	public SimpleJdbcTemplate(DruidDataSource druidDataSource){
		jdbcTemplate = new JdbcTemplate(druidDataSource);
	}
	public SimpleJdbcTemplate(String ip,String port,String dbName,String dbAccount,String dbPassword){
		
		DruidDataSource druidDataSource	= new DruidDataSource();
		druidDataSource.setUrl("jdbc:mysql://"+ip+":"+port+"/"+dbName+"?useUnicode=true&characterEncoding=utf-8");
		druidDataSource.setUsername(dbAccount);
		druidDataSource.setInitialSize(3);
		druidDataSource.setMaxActive(5);
		druidDataSource.setMinIdle(3);
		druidDataSource.setPoolPreparedStatements(false);
		druidDataSource.setDriverClassName("com.mysql.jdbc.Driver");//驱动
		druidDataSource.setUsername(dbAccount);
	...
	public Integer insert(String tablename, Map<String, Object> values) {
		...
		for (String key : values.keySet()) {
			feild = feild + ", `" + key + "` ";
			sql = sql + ", ?";
			param[i++] = values.get(key);
		}
		feild = feild.substring(1, feild.length());
		sql = sql.substring(1, sql.length());
		final String sqlStr = "insert into " + tablename + "(" + feild + ")"
				+ " values " + "(" + sql + ")";
		final Object[] paramFinal = param;
		...
		jdbcTemplate.update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(
				...
				java.sql.PreparedStatement ps = con.prepareStatement(sqlStr,java.sql.Statement.RETURN_GENERATED_KEYS);
				for (int i = 0; i < paramFinal.length; i++) {
					ps.setObject(i + 1, paramFinal[i]);
				}
				return ps;
			}
		}, keyHolder);
