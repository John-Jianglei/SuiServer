...
import com.shinian.qq.open.SnsNetwork;
import com.shinian.qq.open.SnsSigCheck;
import com.shinian.qq.open.SnsStat;
...
public class OpenApiV3
{
     ...
     */
    public String api(String scriptName, HashMap<String, String> params, String protocol) throws OpensnsException, UnsupportedEncodingException
    {
        // 检查openid openkey等参数
        ...
        cookies.put("session_id", URLEncoder.encode("openid", "UTF-8").replace("+", "%20").replace("*", "%2A"));
        cookies.put("session_type",URLEncoder.encode("kp_actoken", "UTF-8").replace("+", "%20").replace("*", "%2A") );
        cookies.put("org_loc",URLEncoder.encode(scriptName, "UTF-8").replace("+", "%20").replace("*", "%2A") );
